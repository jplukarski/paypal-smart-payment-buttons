...
function initVaultCapture({ props, components, payment, serviceData, config } : InitOptions) : PaymentFlowInstance {
    ...
    console.log("Default access token", accessToken);
    ...
    const createAccessTokenWithTargetSubject = (): ZalgoPromise<string> => {
        /* eslint-disable-next-line no-console */
        console.log('createAccessTokenWithTargetSubject()');

        return ZalgoPromise.try(() => {
            return createAccessToken(
                clientID,
                { targetSubject: merchantID[0] }
            ).then(accessTokenWithTargetSubject => {
                /* eslint-disable-next-line no-console */
                console.log("accessTokenWithTargetSubject", accessTokenWithTargetSubject)
                return accessTokenWithTargetSubject;
            }).catch(err => {
    ...
    if (userIDToken && merchantID && merchantID[0]) {
        createAccessTokenWithTargetSubject().then(accessTokenWithTargetSubject => {
            /* eslint-disable-next-line no-console */
            console.log("accessToken from promise", accessTokenWithTargetSubject);
