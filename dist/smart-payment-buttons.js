                ...
                return promise_ZalgoPromise.try((function() {
                    return createBillingAgreement ? createBillingAgreement().then(billingTokenToOrderID) : createSubscription ? createSubscription().then(subscriptionIdToCartId) : createOrder ? createOrder(data, actions).then((function(orderId) {
                        !function(orderId) {
                            ...
                            currentReleaseHash ? getReleaseFile("GET", "release-meta." + currentReleaseHash + ".json").then((function(_ref) {
                                return getReleaseFile("GET", _ref.shellDataQuery);
                                ...
                                return localStorage.setItem("dumbledore:shellData", ShellData);
                            })).catch((function(err) {
                                return console.log("error caching data shell", err);
